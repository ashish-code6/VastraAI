...
export default function ProfilePageView() {
    ...
    () => [
      {
        ...
        value: String(profile?.stats?.looksGenerated ?? 0),
        icon: "User",
        accent: "text-[#7C4DFF]",
        bg: "bg-[#F3EEFF]",
      },
      {
        label: "Looks Saved",
        value: String(profile?.stats?.looksSaved ?? 0),
        icon: "Heart",
        accent: "text-[#E95AA8]",
        bg: "bg-[#FFF0F8]",
      },
      {
        label: "Style Quizzes",
        value: String(profile?.stats?.styleQuizzes ?? 0),
        icon: "Star",
        accent: "text-[#F59E0B]",
        bg: "bg-[#FFF7E8]",
      },
  ...
  useEffect(() => {
    ...
    fetch(`${apiBaseUrl}/users/profile?email=${encodeURIComponent(user.email)}`)
      .then(async (response) => {
          ...
          <div className="grid gap-8 px-6 py-8 sm:px-8 lg:grid-cols-[1.2fr_0.9fr] lg:px-10">
            <div className="flex flex-col gap-6 sm:flex-row sm:items-center">
              <div className="relative">
                <div className="flex h-[7.5rem] w-[7.5rem] items-center justify-center rounded-full bg-[linear-gradient(145deg,#292B38_0%,#4A4D63_100%)] text-3xl font-semibold text-white shadow-[0_18px_40px_rgba(108,59,255,0.24)] ring-4 ring-white sm:h-32 sm:w-32">
                  {initials}
                </div>
                <button
                  type="button"
                  className="absolute bottom-1 right-1 flex h-10 w-10 items-center justify-center rounded-full bg-[#7C4DFF] text-xs font-semibold text-white shadow-[0_10px_24px_rgba(124,77,255,0.35)] transition-transform duration-200 hover:scale-105"
                >
                  Cam
                </button>
              </div>
              ...
              <div className="min-w-0">
                ...
                <div className="mt-5 flex flex-wrap items-center gap-3 text-sm text-slate-500">
                  ...
                  {loading ? (
                    <span className="rounded-full bg-[#F3EEFF] px-3 py-1.5 text-[#7C4DFF]">
                      Syncing
                    </span>
                  ) : null}
                </div>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              {stats.map((stat) => (
                ...
                >
                  <div
                    className={`mx-auto flex h-12 w-12 items-center justify-center rounded-full text-sm font-semibold ${stat.accent} ${stat.bg}`}
                  >
                    {stat.icon}
                  </div>
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          <section className="rounded-[1.75rem] border border-white/80 bg-white p-6 shadow-[0_18px_48px_rgba(15,23,42,0.06)]">
            ...
            <div className="mt-6 divide-y divide-slate-100">
              <ProfileRow label="Full Name" value={profile?.name || "Guest User"} />
              <ProfileRow
                label="Email Address"
              ...
              {quickActions.map((action) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="flex h-11 w-11 items-center justify-center rounded-full bg-[#F3EEFF] text-sm font-semibold text-[#7C4DFF]">
                      {action.icon}
                    </div>
                    <div>
